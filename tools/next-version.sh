...
# MAJOR, a feat is MINOR, anything else (fix, perf, ...) is PATCH. Commits that
# change nothing a user sees (docs, test, ci, style, chore, build, refactor)
...
if [ -z "$last" ]; then
    base=$(sed -n 's/^__version__ = "\(.*\)"/\1/p' */__init__.py | head -1)
    range="HEAD"
else
    base="${last#v}"
    range="$last..HEAD"
