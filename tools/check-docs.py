...
does not go out while it fails (docs/RELEASING.md).
  ...
  2. Every docs/*.md is listed in docs/README.md, so nothing becomes a file
     nobody will find.
  3. docs/ holds Markdown only; artwork belongs in data/repo/.
  ...
  6. The newest CHANGELOG version equals __version__ in the package.
...
SKIP_DIRS = {".git", ".venv", "venv", "node_modules", "claude-memory-hylki", ".claude", ".pytest_cache", "var"}
...
def check_docs_dir() -> None:
    ...
    for path in docs.rglob("*"):
        if path.is_file() and path.suffix != ".md":
            fail(f"{path.relative_to(ROOT)}: docs/ holds Markdown only; artwork goes in data/repo/")
...
def check_version(newest: str | None) -> None:
    inits = [p for p in ROOT.glob("*/__init__.py") if p.parent.name not in ("tests", "tools")]
    if len(inits) != 1:
        fail(f"expected one package directory, found {len(inits)}")
        return
    m = re.search(r'^__version__\s*=\s*"([^"]+)"', inits[0].read_text(), re.M)
    if not m:
        fail(f"{inits[0].relative_to(ROOT)} has no __version__")
